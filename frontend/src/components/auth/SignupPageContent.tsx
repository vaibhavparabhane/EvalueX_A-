...
import { Loader2, Mail, Lock, User, CheckCircle2, Sparkles } from 'lucide-react';
...
const benefits = [
  'Grade 10× faster with AI-powered OCR',
  'Consistent rubric-based evaluation',
  'Instant AI feedback for every student',
  'Full educator override & control',
];
  ...
  return (
    <div className="min-h-screen flex">
        ...
        <div className="relative z-10 space-y-8">
          <div className="space-y-4">
            ...
            <h1 className="text-[2.6rem] font-black leading-tight" style={{ color: 'white', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
              Grade Smarter,<br />
              <span style={{ color: 'hsl(173,80%,75%)' }}>Not Harder.</span>
            </h1>

            <p className="text-base font-medium leading-relaxed max-w-xs" style={{ color: 'rgba(255,255,255,0.72)' }}>
              Join educators worldwide using EvalueX to automate handwritten assessment evaluation with AI.
            </p>
          </div>
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12 relative"
        ...
        <div className="absolute top-0 right-0 w-72 h-72 rounded-full blur-[140px] pointer-events-none opacity-30"
          style={{ background: 'hsl(173,58%,39%)' }} />
        <div className="absolute bottom-0 left-0 w-60 h-60 rounded-full blur-[120px] pointer-events-none opacity-20"
        ...
        <div className="w-full max-w-[420px] space-y-7 relative z-10">
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm font-medium" style={{ color: 'hsl(215,16%,47%)' }}>
              Already have one?{' '}
              <Link href="/login" className="font-semibold hover:underline" style={{ color: 'hsl(173,58%,35%)' }}>
                Sign in
              </Link>
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5 text-left">
              <label htmlFor="fullName" className="block text-sm font-semibold" style={{ color: 'hsl(222,47%,20%)' }}>
                Full Name
              </label>
              <div className="relative">
                <User className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4" style={{ color: 'hsl(215,16%,55%)' }} />
                <Input
                  id="fullName"
                  type="text"
            ...
            <div className="space-y-1.5 text-left">
              ...
              <div className="mt-2 p-3 bg-slate-100/50 rounded-xl border border-slate-200/50 space-y-1 text-xs">
                <p className="font-semibold text-slate-500 mb-1.5">Password requirements:</p>
                <div className="grid grid-cols-1 gap-1">
                  <div className="flex items-center gap-1.5 font-medium">
                    <span className={`h-1.5 w-1.5 rounded-full transition-colors duration-200 ${password.length >= 8 ? 'bg-emerald-500' : (password.length === 0 ? 'bg-slate-300' : 'bg-rose-400')}`} />
                    <span className={password.length >= 8 ? 'text-emerald-600 dark:text-emerald-400 line-through opacity-65 transition-all' : 'text-slate-600 transition-all'}>
                      Minimum 8 characters
                    </span>
                  </div>
                  <div className="flex items-center gap-1.5 font-medium">
                    <span className={`h-1.5 w-1.5 rounded-full transition-colors duration-200 ${/[A-Z]/.test(password) ? 'bg-emerald-500' : (password.length === 0 ? 'bg-slate-300' : 'bg-rose-400')}`} />
                    <span className={/[A-Z]/.test(password) ? 'text-emerald-600 dark:text-emerald-400 line-through opacity-65 transition-all' : 'text-slate-600 transition-all'}>
                  ...
                  <div className="flex items-center gap-1.5 font-medium">
                    <span className={`h-1.5 w-1.5 rounded-full transition-colors duration-200 ${/[0-9]/.test(password) ? 'bg-emerald-500' : (password.length === 0 ? 'bg-slate-300' : 'bg-rose-400')}`} />
                    <span className={/[0-9]/.test(password) ? 'text-emerald-600 dark:text-emerald-400 line-through opacity-65 transition-all' : 'text-slate-600 transition-all'}>
                      At least one number (0-9)
                    </span>
                  </div>
                  <div className="flex items-center gap-1.5 font-medium">
                    <span className={`h-1.5 w-1.5 rounded-full transition-colors duration-200 ${/[^A-Za-z0-9]/.test(password) ? 'bg-emerald-500' : (password.length === 0 ? 'bg-slate-300' : 'bg-rose-400')}`} />
                    <span className={/[^A-Za-z0-9]/.test(password) ? 'text-emerald-600 dark:text-emerald-400 line-through opacity-65 transition-all' : 'text-slate-600 transition-all'}>
            ...
            <button
              ...
              style={{
                ...
                boxShadow: '0 4px 20px rgba(45,165,142,0.30)'
              }}
              onMouseEnter={e => (e.currentTarget.style.boxShadow = '0 6px 28px rgba(45,165,142,0.45)')}
              onMouseLeave={e => (e.currentTarget.style.boxShadow = '0 4px 20px rgba(45,165,142,0.30)')}
            >
          ...
          <p className="text-xs text-center leading-relaxed" style={{ color: 'hsl(215,16%,57%)' }}>
            ...
            <span className="cursor-pointer hover:underline font-medium" style={{ color: 'hsl(173,58%,35%)' }}>Terms of Service</span>
            {' '}and{' '}
            <span className="cursor-pointer hover:underline font-medium" style={{ color: 'hsl(173,58%,35%)' }}>Privacy Policy</span>.
          </p>
        </div>
      </div>
    </div>
